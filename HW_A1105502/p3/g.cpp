...
int main()
{
    ...
    for (i = 0; i < biHeight; i++)
    {
        for (j = 0; j < biWidth; j++)
        {
            if (data1[i * biWidth4 + j] + new_data[i * biWidth4 + j] > 255)
            {
                data2[i * biWidth4 + j] = 255;
            }
            else
            {
                data2[i * biWidth4 + j] = data1[i * biWidth4 + j] + new_data[i * biWidth4 + j];
            }
        }
    }
    //

    if ((output_file = fopen("g.bmp", "wb")) == NULL)
    {
