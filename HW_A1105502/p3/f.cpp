...
int main()
{
    ...
    ReadImageData("c.bmp", bmpfileheader1, bmpinfoheader1, color_table1, data1);
    ReadImageData("e.bmp", bmpfileheader1, bmpinfoheader1, color_table1, new_data);
    ...
    for (i = 0; i < biHeight; i++)
    {
        for (j = 0; j < biWidth; j++)
        {
            // Product data1 and new_data
            data2[i * biWidth4 + j] = data1[i * biWidth4 + j] * new_data[i * biWidth4 + j] / 225;
        }
    }
    //

    if ((output_file = fopen("f.bmp", "wb")) == NULL)
    {
