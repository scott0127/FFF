...
int main()
{
    ...
    U_CHAR *data1, *data2, *original_data, color_table1[1024], color_table2[1024];
    ...
    i = ReadDataSize("averaging.bmp");
    data1 = (U_CHAR *)malloc(i);
    if (data1 == NULL)
    {
        exit(0);
    }
    i = ReadDataSize("Fig0338(a).bmp");
    original_data = (U_CHAR *)malloc(i);
    if (original_data == NULL)
    {
        exit(0);
    }

    ReadImageData("averaging.bmp", bmpfileheader1, bmpinfoheader1, color_table1, data1);
    ReadImageData("Fig0338(a).bmp", bmpfileheader1, bmpinfoheader1, color_table1, original_data);
    ...
    for (i = 0; i < biHeight; i++)
    {
        ...
        {
            int sum = 0;
            // 輪每個像素進行 Laplacian kernel
            for (int mi = -1; mi <= 1; mi++)
            {
                for (int mj = -1; mj <= 1; mj++)
                {
                    int ni, nj;
                    correctedPos(&ni, &nj, i + mi, j + mj, biHeight, biWidth);
                    // Apply kernel
                    sum += data1[ni * biWidth4 + nj] * kernel[mi + 1][mj + 1];
                }
            }
            //基本上跟a有點像，先找出blur version 然後照課本公式， 原圖-模糊，就是銳化，而unsharp跟highboost則是銳化強度的差別 ex k=4銳化程度4
            
            if (sum < 0)
                sum = 0;
            int k = i * biWidth4 + j;
            if ((int)original_data[k] - sum < 0)
                data2[k] = 0;
            else if ((int)original_data[k] - sum > 255)
                data2[k] = 255;
            else
                data2[k] = original_data[k] - sum;
        }
    }

    if ((output_file = fopen("p2-b.bmp", "wb")) == NULL)
    {
