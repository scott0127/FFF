...
int main()
{
    ...
    ReadImageData("Fig0338(a).bmp", bmpfileheader1, bmpinfoheader1, color_table1, data1);
    ...
    for (i = 0; i < biHeight; i++)
    {
        ...
        {
            int sum = 0;
            //原本的average不可用，因為他是去掉第一排，然後加入下一排，可是laplacian kernel 並非每一行都一樣
            ...
            //要拿sum做有沒有overflow 因為 如果先給值到data2 但data2 是 unsigned char
            if (sum < 0)
            {
                sum = 0;
            }
    ...
    if ((output_file = fopen("p2-a.bmp", "wb")) == NULL)
    {
